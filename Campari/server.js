...
url = 'file://' + fs.absolute('./canvas.html');

function drawRecipe(recipe) {

    var ingredients = recipe.ingredients;
    var glass = recipe.glassware;
    ...
    return [document.getElementById(selector).innerHTML, document.getElementById(selector).getBoundingClientRect()];       
}

var service = server.listen(port, function (request, response) {
   var recipe = JSON.parse(request.post);
   page.open(url, function (status) {
      var result = page.evaluate(drawRecipe, recipe);
      var svg = result[0];
      page.clipRect = result[1];
      page.render('output/png/' + recipe.slug + '.png');
